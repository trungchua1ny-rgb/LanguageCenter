  ...
  <div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
        <h4 class="fw-bold mb-1">
          <i class="bi bi-award text-warning me-2"></i>Quản lý Điểm số
        </h4>
    ...
    <c:if test="${param.error == 'invalid_score'}">
      <div class="alert alert-danger alert-dismissible fade show" role="alert">
        <i class="bi bi-exclamation-triangle-fill me-2"></i>Điểm không hợp lệ. Điểm phải &ge; 0 và không vượt quá điểm tối đa.
        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
      </div>
    </c:if>
    ...
    <div class="card border-0 shadow-sm mb-4">
      <div class="card-body">
        <form method="get" action="${pageContext.request.contextPath}/admin/grades" class="row g-3 align-items-end">
          <div class="col-md-6">
            <label class="form-label fw-semibold">
              <i class="bi bi-funnel me-1"></i>Lọc theo lớp học
            </label>
          ...
          <div class="col-md-2">
            <a href="${pageContext.request.contextPath}/admin/grades" class="btn btn-outline-secondary w-100">
            ...
            <div class="card border-0 shadow-sm">
              <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
                <h6 class="fw-bold mb-0">
                  <i class="bi bi-table me-2 text-warning"></i>Bảng điểm — Lớp #<strong>${selectedClassId}</strong>
                </h6>
                      ...
                      <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                          <c:forEach var="g" items="${grades}">
                            <tr>
                              <td>
                                <span class="fw-semibold">${g.studentName}</span>
                              </td>
                              <td>${g.examName}</td>
                              ...
                              <td class="text-center">
                                <div class="progress" style="height:6px; min-width:60px;">
                                  <div class="progress-bar ${g.rankBadgeClass}" style="width:${g.scorePercent}%"></div>
                                </div>
                                <small class="text-muted">${g.scorePercent}%</small>
                              </td>
                              <td>
                                <span class="badge ${g.rankBadgeClass} rounded-pill">
                                  ${g.rank}
                                </span>
                              </td>
                              ...
                              <td class="text-center">
                                <a href="${pageContext.request.contextPath}/admin/grades?classId=${selectedClassId}&editId=${g.id}"
                                   class="btn btn-sm btn-outline-primary me-1" title="Sửa điểm">
          ...
          <div class="col-lg-4">
            ...
            <c:if test="${not empty editGrade}">
              <div class="card border-warning border-2 shadow-sm mb-3">
                <div class="card-header bg-warning bg-opacity-10 border-warning-subtle py-3">
                  <h6 class="fw-bold mb-0 text-warning">
                    <i class="bi bi-pencil-square me-2"></i>Sửa điểm
                  </h6>
                </div>
                <div class="card-body">
                  <form method="post" action="${pageContext.request.contextPath}/admin/grades?action=update" class="needs-validation" novalidate>
                    <input type="hidden" name="classId" value="${selectedClassId}">
                    <input type="hidden" name="id" value="${editGrade.id}">

                    <div class="mb-2">
                      <label class="form-label fw-semibold small">Học viên</label>
                      <input type="text" class="form-control form-control-sm" value="${editGrade.studentName}" disabled>
                    </div>
                    <div class="mb-2">
                      <label class="form-label fw-semibold small">Tên bài thi <span class="text-danger">*</span></label>
                      <input type="text" name="examName" class="form-control form-control-sm" value="${editGrade.examName}" required maxlength="100">
                    </div>
                    <div class="row g-2 mb-2">
                      <div class="col-6">
                        <label class="form-label fw-semibold small">Điểm đạt <span class="text-danger">*</span></label>
                        <input type="number" name="score" class="form-control form-control-sm" value="${editGrade.score}" step="0.1" min="0" required>
                      </div>
                      <div class="col-6">
                        <label class="form-label fw-semibold small">Điểm tối đa <span class="text-danger">*</span></label>
                        <input type="number" name="maxScore" class="form-control form-control-sm" value="${editGrade.maxScore}" step="0.1" min="0.1" required>
                      </div>
                    </div>
                    <div class="mb-2">
                      <label class="form-label fw-semibold small">Ngày thi</label>
                      <input type="date" name="examDate" class="form-control form-control-sm" value="${editGrade.examDate}">
                    </div>
                    <div class="mb-3">
                      <label class="form-label fw-semibold small">Ghi chú</label>
                      <input type="text" name="note" class="form-control form-control-sm" value="${editGrade.note}" maxlength="255">
                    </div>
                    <div class="d-flex gap-2">
                      <button type="submit" class="btn btn-warning btn-sm flex-grow-1">
                        <i class="bi bi-save me-1"></i>Lưu thay đổi
                      </button>
                      <a href="${pageContext.request.contextPath}/admin/grades?classId=${selectedClassId}" class="btn btn-outline-secondary btn-sm">
                        <i class="bi bi-x"></i>
                      </a>
                    </div>
                  </form>
            ...
            <div class="card border-0 shadow-sm">
              ...
              <div class="card-body">
                <form method="post" action="${pageContext.request.contextPath}/admin/grades?action=insert" class="needs-validation" novalidate>
                  <input type="hidden" name="classId" value="${selectedClassId}">

                  <div class="mb-3">
                    <label class="form-label fw-semibold small">Chọn học viên <span class="text-danger">*</span></label>
                  ...
                  <div class="mb-3">
                    <label class="form-label fw-semibold small">Tên bài thi <span class="text-danger">*</span></label>
                    <input type="text" name="examName" class="form-control form-control-sm" placeholder="VD: Kiểm tra giữa kỳ" required maxlength="100">
                  ...
                  <div class="row g-2 mb-3">
                    <div class="col-6">
                      <label class="form-label fw-semibold small">Điểm đạt <span class="text-danger">*</span></label>
                      <input type="number" name="score" class="form-control form-control-sm" placeholder="VD: 8.5" step="0.1" min="0" required>
                      <div class="invalid-feedback">Bắt buộc.</div>
                    </div>
                    <div class="col-6">
                      <label class="form-label fw-semibold small">Thang điểm <span class="text-danger">*</span></label>
                      <input type="number" name="maxScore" class="form-control form-control-sm" placeholder="VD: 10" step="0.1" min="0.1" value="10" required>
                      <div class="invalid-feedback">Bắt buộc.</div>
                    </div>
                  </div>
                  ...
                  <div class="mb-4">
                    <label class="form-label fw-semibold small">Ghi chú</label>
                    <input type="text" name="note" class="form-control form-control-sm" placeholder="Tùy chọn" maxlength="255">
                  </div>
  ...
  <div class="modal-dialog modal-sm">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <form id="deleteForm" method="post" action="${pageContext.request.contextPath}/admin/grades?action=delete">
          <input type="hidden" name="classId" value="${selectedClassId}">
